...
export function usePageCollaboration(){
  ...
  function connect(options:{pageId:string;initialBody:string;userId:string;email:string}){
    ...
    let stopped=false,socket:WebSocket|null=null,reconnectTimer=0,heartbeatTimer=0,attempt=0,ready=false,sessionId:string=crypto.randomUUID();let selection:{start:number;end:number}|undefined
