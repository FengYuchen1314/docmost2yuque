...
async function createNote() {
  ...
  try { await post<QuickNote>('/api/v1/quick-notes/create', { workspaceId: session.activeWorkspace.id, content: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: capture.value.trim() }] }] }, plainText: capture.value.trim(), source: 'HOME', clientRequestId: createUuid(), tagIds: [] }); capture.value = ''; ui.notify('已保存到小记') }
