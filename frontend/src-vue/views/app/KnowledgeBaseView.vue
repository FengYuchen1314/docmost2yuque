...
<template>
  <div class="page-shell">
    <header class="page-heading"><div class="d-flex align-center"><v-avatar color="primary" variant="tonal" rounded="lg" class="mr-4">{{ knowledgeBase?.icon || '📘' }}</v-avatar><div><h1>{{ knowledgeBase?.name || '知识库' }}</h1><p>{{ knowledgeBase?.description || '集中管理文档、白板和结构化数据。' }}</p></div></div><div class="d-flex ga-2"><v-btn variant="outlined" prepend-icon="mdi-chart-box-outline" @click="analyticsOpen = true">统计</v-btn><v-btn :to="`/app/kb/${id}/settings`" variant="outlined" prepend-icon="mdi-cog-outline">设置</v-btn><v-btn color="primary" prepend-icon="mdi-plus" @click="ui.createOpen = true">新建内容</v-btn></div></header>
    <v-card class="section-card">
      ...
      <div v-else-if="!loading" class="empty-state"><div><v-icon size="48">mdi-folder-open-outline</v-icon><h3>{{ query ? '没有匹配的内容' : '知识库还是空的' }}</h3><p>{{ query ? '换一个关键词试试。' : '创建第一篇文档、白板、电子表格或数据表。' }}</p><v-btn v-if="!query" color="primary" prepend-icon="mdi-plus" @click="ui.createOpen = true">创建内容</v-btn></div></div>
    </v-card>
