...
watch(id, (knowledgeBaseId) => {
  ++routeContextSequence
  ++loadSequence
  ++deleteSequence
  ++duplicateSequence
  ...
  duplicatingId.value = ''
  void load(knowledgeBaseId)
}, { immediate: true })

async function load(requestedKnowledgeBaseId = id.value) {
  const context = routeContextSequence
  const request = ++loadSequence
  loading.value = true
  error.value = ''
  loadError.value = ''
  try {
    const [nextKnowledgeBase, nextPages] = await Promise.all([
      post<KnowledgeBase>('/api/v1/knowledge-bases/get', { knowledgeBaseId: requestedKnowledgeBaseId }),
      post<Page[]>('/api/v1/pages/list', { knowledgeBaseId: requestedKnowledgeBaseId }),
    ])
    if (!isCurrentContext(requestedKnowledgeBaseId, context) || request !== loadSequence) return
    if (nextKnowledgeBase.id !== requestedKnowledgeBaseId || nextPages.some((page) => page.knowledgeBaseId !== requestedKnowledgeBaseId)) {
      throw new Error('知识库响应与当前页面不一致，请重新加载')
    }
    if (nextKnowledgeBase.homepagePageId) {
      await router.replace(knowledgeBaseDestination(nextKnowledgeBase))
      return
    }
    knowledgeBase.value = nextKnowledgeBase
    ...
    if (isCurrentContext(requestedKnowledgeBaseId, context) && request === loadSequence) loading.value = false
  }
}

function isCurrentContext(knowledgeBaseId: string, context: number) {
  return id.value === knowledgeBaseId && routeContextSequence === context
}
...
async function remove() {
  ...
  const requestedKnowledgeBaseId = id.value
  if (page.knowledgeBaseId !== requestedKnowledgeBaseId) {
    deleteTarget.value = null
    error.value = '页面上下文已变化，请重新选择后再试'
    return
  }
  const context = routeContextSequence
  const request = ++deleteSequence
  deleting.value = true
  error.value = ''
  try {
    await post('/api/v1/pages/trash', { pageId: page.id })
    if (!isCurrentContext(requestedKnowledgeBaseId, context) || request !== deleteSequence) return
    deleteTarget.value = null
    ui.notify('已移入回收站')
    await load(requestedKnowledgeBaseId)
  } catch (value) {
    if (isCurrentContext(requestedKnowledgeBaseId, context) && request === deleteSequence) error.value = messageOf(value)
  } finally {
    if (isCurrentContext(requestedKnowledgeBaseId, context) && request === deleteSequence) deleting.value = false
  }
}
...
async function duplicate(page: Page) {
  ...
  const context = routeContextSequence
  const request = ++duplicateSequence
  duplicatingId.value = page.id
  error.value = ''
  try {
    const copy = await post<Page>('/api/v1/pages/copy', {
      pageId: page.id,
      targetKnowledgeBaseId: requestedKnowledgeBaseId,
      title: `${page.title} 副本`,
      path: `${page.path.replace(/-copy(?:-\d+)?$/, '')}-copy-${Date.now().toString(36)}`.slice(0, 180),
    })
    if (!isCurrentContext(requestedKnowledgeBaseId, context) || request !== duplicateSequence) return
    await load(requestedKnowledgeBaseId)
    if (!isCurrentContext(requestedKnowledgeBaseId, context) || request !== duplicateSequence) return
    ui.notify('副本已创建')
    await router.push(`/app/kb/${requestedKnowledgeBaseId}/pages/${copy.id}`)
  } catch (value) {
    if (isCurrentContext(requestedKnowledgeBaseId, context) && request === duplicateSequence) error.value = messageOf(value)
  } finally {
    if (isCurrentContext(requestedKnowledgeBaseId, context) && request === duplicateSequence) duplicatingId.value = ''
  }
}
...
<template>
  <div class="knowledge-base-page">
    ...
    <section v-if="catalogManage" class="catalog-manager-stage">
      ...
      <CatalogPanel class="catalog-manager" :knowledge-base-id="id" :pages="pages" show-history @page-created="reloadCurrent" @page-trashed="reloadCurrent" @open-page="openPage" />
    </section>
    ...
    <div v-else class="knowledge-layout">
      ...
      <main class="knowledge-content">
        ...
        <div v-if="loadError && !loading" class="content-load-error" role="alert">
          <span class="empty-illustration"><v-icon size="34">mdi-cloud-alert-outline</v-icon></span>
          <h3>知识库加载失败</h3>
          <p>{{ loadError }}</p>
          <v-btn color="primary" size="small" prepend-icon="mdi-refresh" @click="reloadCurrent">重新加载</v-btn>
        </div>

        <div v-else-if="filtered.length && displayMode === 'list'" class="content-list">
...
<style scoped>
.content-load-error{display:flex;min-height:360px;align-items:center;justify-content:center;flex-direction:column;color:#8a8f8d;text-align:center}
.content-load-error h3{margin:16px 0 4px;color:#3d424a;font-size:15px;font-weight:500}
.content-load-error p{max-width:460px;margin:0 0 14px;font-size:12px;line-height:19px}
.row-menu,.content-card .card-menu{opacity:1}
@media(hover:hover) and (pointer:fine){.row-menu,.content-card .card-menu{opacity:0}.content-row:hover .row-menu,.content-row:focus-within .row-menu,.content-card:hover .card-menu,.content-card:focus-within .card-menu{opacity:1}}
</style>
