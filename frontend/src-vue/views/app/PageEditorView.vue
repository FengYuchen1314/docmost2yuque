...
const title=ref('');const body=ref('');const loading=ref(true);const error=ref('');const status=ref<'saved'|'dirty'|'saving'|'offline'|'conflict'>('saved')
...
<template>
  <div class="editor-page">
    <v-toolbar color="surface" flat border="bottom" height="64" class="editor-header px-2">
      ...
      <v-tooltip :text="collaboration.error.value || `实时协作：${collaboration.status.value}`"><template #activator="{ props }"><v-chip v-bind="props" :color="collaboration.status.value==='connected'?'success':'warning'" variant="tonal" size="small" class="mr-2"><v-icon start size="14">mdi-account-multiple-outline</v-icon>{{collaboration.peers.value.length ? `${collaboration.peers.value.length + 1} 人` : collaboration.status.value==='connected' ? '在线' : '连接中'}}</v-chip></template></v-tooltip>
    ...
    <template v-if="page&&!loading"><DocumentEditor v-if="page.contentType==='DOCUMENT'" ref="documentEditor" v-model="body" :document-settings="page.documentSettings" @blur="save" @selection-change="collaboration.broadcastSelection"/><StructuredEditor v-else :type="page.contentType" v-model="body"/></template>
...
<style scoped>
.editor-page{height:100vh;overflow:auto;background:rgb(var(--v-theme-surface))}.editor-header{position:sticky!important;top:0;z-index:30}.editor-page :deep(.editor-toolbar){top:64px}.editor-title{font-size:16px;font-weight:650;max-width:min(520px,40vw)}
</style>
